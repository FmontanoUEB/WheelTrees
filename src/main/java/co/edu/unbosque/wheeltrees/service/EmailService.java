...
public class EmailService {
	...
	@Async
	public void enviarCodigoRecuperacion(String destinatario, String nombre, String otp) {
		log.info("📧 Preparando envío de recuperación de contraseña hacia: {}", destinatario);

		String link = resetPasswordUrl + "?email=" + destinatario + "&codigo=" + otp;
		...
		try {
			...
			helper.setFrom("noreply@wheeltrees.site");
			helper.setTo(destinatario);
			helper.setSubject("Recupera tu contraseña — Wheel-Trees");
			helper.setText(construirTextoPlanoRecuperacion(nombre, otp, link),
					construirHtmlRecuperacion(nombre, otp, link));

			mailSender.send(message);
			log.info("✅ Correo de recuperación enviado exitosamente a: {}", destinatario);

		} catch (MessagingException e) {
			log.error("❌ Error enviando correo de recuperación con SMTP: {}", e.getMessage());
		}
	}

	private String construirTextoPlanoRecuperacion(String nombre, String otp, String link) {
		return "Hola " + nombre + ". Solicitaste recuperar tu contraseña en Wheel-Trees. Tu código es: " + otp
				+ ". También puedes usar este enlace: " + link + ". El código expira en 15 minutos. "
				+ "Si no fuiste tú, ignora este mensaje.";
	}

	private String construirHtmlRecuperacion(String nombre, String otp, String link) {
		return """
				<!DOCTYPE html>
				<html lang="es">
				<head>
				    ...
				    <title>Recuperar contraseña Wheel-Trees</title>
				</head>
				        ...
				        <table width="100%%" style="max-width:480px;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e0e0e0;">
				          ...
				          <tr>
				            <td style="padding:36px 32px 28px;">
				              ...
				              <p style="font-size:14px;color:#666666;line-height:1.7;margin:0 0 24px;">
				                Recibimos una solicitud para restablecer tu contraseña. Toca el botón para continuar,
				                o usa el código manualmente si te lo pide la aplicación.
				              </p>
				                ...
				                <tr>
				                  <td style="padding:10px 14px;">
				                    <p style="font-size:12px;color:#9a6820;margin:0;line-height:1.6;">
				                      &#9432; Este código y enlace expiran en <strong>15 minutos</strong>. Si no solicitaste esto, ignora el mensaje.
				                    </p>
				                  </td>
				                </tr>
				              </table>
